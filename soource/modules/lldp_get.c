...
int lldpmod_snmpwalk_getrem_cb(void *arg, const netsnmp_variable_list *var)
{
		...
		getrem_data->rem_index == var->name[var->name_length - 1])
	{
		if (SUCCEED != (err = lldpmod_snmp_set_result(var, getrem_data->result, getrem_data->res_type, getrem_data->dump_flag)))
		{
			...
			zabbix_log(LOG_LEVEL_DEBUG, "%s: cannot get index '%s' value: %s", MODULE_NAME,
					getrem_data->index, getrem_data->rem_index, NULL != msg && NULL != *msg ? *msg : "(null)");
		...
		NULL != GET_STR_RESULT(&snmp_result))
	{
		...
		if (NULL != obj)
		{
			if (data->request.key_types[data->num] == KEY_TYPE_REM)
			{
				...
				{
					zabbix_log(LOG_LEVEL_WARNING, "%s: the same index appears again. oid='%s',index=%ld, rem_index=%ld, oldvalue='%s', newvalue='%s'",
						MODULE_NAME,
			...
			else	/* KEY_TYPE_LOC */
			{
				...
				{
					zabbix_log(LOG_LEVEL_WARNING, "%s: the same index appears again. oid='%s',index=%ld, oldvalue='%s', newvalue='%s'",
						MODULE_NAME,
						data->request.oids[data->num],
						index,
						obj->values[data->num],
	...
	while (NULL != (obj = (lldpmod_snmp_object_t *)zbx_hashset_iter_next(&iter)))
	{
		if (0 == strcmp(param_func, "remote"))
		{
			...
			if (NULL == obj->rem_objects || 0 == obj->rem_objects->num_data)
			{
				...
				zbx_snprintf(oid_str_buf, sizeof(oid_str_buf), "%lu", obj->index);
			...
			else
			{
				...
				while (NULL != (rem_obj = (lldpmod_snmp_object_t *)zbx_hashset_iter_next(&rem_iter)))
				{
					...
					zbx_snprintf(oid_str_buf, sizeof(oid_str_buf), "%lu", rem_obj->index);
...
int lldpmod_getrem_common(AGENT_REQUEST *request, AGENT_RESULT *result, const char *get_mib, lldpmod_getrem_data_t *getrem_data)
{
	...
	getrem_data->index = (oid)atol(param_port_num);
	getrem_data->rem_index = (oid)atol(param_rem_idx);
...
exit:
	if (SUCCEED_WITH_VALUE != err)
	{
		if (SUCCEED == err)
		{
			...
			SET_MSG_RESULT(result, zbx_dsprintf(NULL, "Object not found. [ %s, %ld, %ld ]",
					get_mib, getrem_data->index, getrem_data->rem_index));
		}
		else
		{
			SET_MSG_RESULT(result, zbx_strdup(NULL, error));
...
exit:
	if (SUCCEED_WITH_VALUE != err)
	{
		if (SUCCEED == err)
		{
			/* specified remote index was not found  */
			SET_MSG_RESULT(result, zbx_dsprintf(NULL, "Object not found. [ %s, %ld, %ld ]",
					param_oid, getrem_data.index, getrem_data.rem_index));
		}
		else
		{
			SET_MSG_RESULT(result, zbx_strdup(NULL, error));
